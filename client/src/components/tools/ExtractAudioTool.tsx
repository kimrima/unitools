...
import { extractAudio, FFmpegError, isFFmpegSupported, type ExtractAudioResult } from '@/lib/engines/ffmpegEngine';
import { getFileSizeData } from '@/lib/engines/imageCompress';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { Progress } from '@/components/ui/progress';
import { Video, Upload, Download, Loader2, Music, AlertTriangle } from 'lucide-react';
...
export default function ExtractAudioTool() {
  ...
  const handleDownload = useCallback(() => {
    ...
    downloadBlob(result.outputBlob, `unitools_${baseName}.mp3`);
  }, [result]);
  ...
  return (
    <div className="space-y-6">
      <div className="text-sm text-muted-foreground" data-testid="text-instructions">
        {t('Tools.extract-audio.instructions')}
      </div>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <video
              src={videoUrl}
              controls
              className="w-full max-h-48 mt-4 rounded-lg"
      ...
      {result && (
        <Card className="bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-800">
          <CardContent className="p-4 space-y-4">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <div>
                ...
                <p className="text-sm text-green-600 dark:text-green-400">
                  {formatFileSize(result.originalSize)} → {formatFileSize(result.outputSize)} (MP3)
                </p>
              </div>
            ...
            <audio
              src={URL.createObjectURL(result.outputBlob)}
              controls
              className="w-full"
