...
import { applyGrayscale } from '@/lib/engines/imageEngine';
import { Button } from '@/components/ui/button';
import { Card } from '@/components/ui/card';
import { Progress } from '@/components/ui/progress';
import { Upload, Download, Loader2, CheckCircle, Contrast } from 'lucide-react';
...
export default function GrayscaleFilterTool() {
  ...
  const handleApply = useCallback(async () => {
    ...
    setError(null);
    try {
      const blob = await applyGrayscale(
        files[0].previewUrl,
      ...
      setError({ code: 'GRAYSCALE_FAILED' });
      setStatus('error');
    }
  ...
  const handleDownload = useCallback(() => {
    ...
    downloadResult(`unitools_grayscale.${ext}`);
  }, [files, downloadResult]);

  return (
    <div className="space-y-6">
      <div className="text-sm text-muted-foreground">{t('Tools.grayscale-filter.description')}</div>
      ...
      {status === 'idle' && files.length === 0 && (
        <div onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} onClick={() => fileInputRef.current?.click()}
          ...
          <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center">
            <Contrast className="w-8 h-8 text-primary" />
          </div>
      ...
      {status === 'idle' && files.length > 0 && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Card className="p-4">
              <p className="text-sm font-medium mb-2">{t('Tools.grayscale-filter.original', { defaultValue: 'Original' })}</p>
              {files[0].previewUrl && <img src={files[0].previewUrl} alt="Original" className="w-full h-48 object-contain bg-muted rounded" />}
            </Card>
            <Card className="p-4">
              <p className="text-sm font-medium mb-2">{t('Tools.grayscale-filter.preview', { defaultValue: 'Preview' })}</p>
              {files[0].previewUrl && <img src={files[0].previewUrl} alt="Preview" className="w-full h-48 object-contain bg-muted rounded grayscale" />}
            </Card>
          </div>
          <div className="flex gap-3">
            <Button onClick={handleApply} className="flex-1">{t('Tools.grayscale-filter.title')}</Button>
