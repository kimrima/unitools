...
export default function PdfToPngTool() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      await stagedProcessing.runStagedProcessing(async () => {
        const options = { format: 'png' as const, scale };
        ...
        if (images.length > 1) {
          const zip = await imagesToZip(images, 'png', baseName);
          setZipBlob(zip);
        }
  ...
  }, [files, scale, setStatus, setError, stagedProcessing]);
  ...
  const handleDownloadSingle = useCallback((blob: Blob, index: number) => {
    ...
    a.href = url;
    a.download = `${baseName}_page_${index + 1}.png`;
    a.click();
  ...
  return (
    <div className="space-y-6">
      <div className="text-sm text-muted-foreground" data-testid="text-instructions">
        {t('Tools.pdf-to-png.description')}
      </div>
